...
SqlGraph 通过静态解析一整个目录的 SQL，构建出一张属性图。它不仅刻画 *表/字段血缘*，更把 **加工逻辑本身** 建模为图节点。跨 SQL 的相同逻辑会通过内容指纹收敛为 **同一个共享节点** —— 让你一眼看清仓库里哪些计算被重复使用。
...
- **相同逻辑自动去重。** 两个不同 SQL 文件里的同一个表达式会解析成**同一个**节点（相同的 128-bit 内容指纹）。交换律算子被归一化，因此 `a + b` 与 `b + a` 收敛。
...
| **内容指纹 DAG** | Merkle 式的加工逻辑全局去重 |
| **多种输出** | 交互式 HTML、CSV（节点/边）、GraphRAG JSON、纯 JSON、NetworkX |
| **交互式可视化** | Cytoscape.js + ELK 分层布局，浅色/深色主题、搜索、节点大小调节、PNG/SVG 导出 |
...
1. **Input** —— 从文件、目录、字符串或 DataFrame 发现 SQL；可选加载 `schema.csv` 用于字段消歧。
2. **Parser** —— SQLGlot 构建 AST；`ColumnResolver` 把每个列绑定到物理 `table.column`。[表达式 DAG 模块](sqlgraph/parser/expr_dag.py) 把每个输出表达式转成带指纹的逻辑节点。
3. **Builder** —— [图构建器](sqlgraph/builder/graph_builder.py) 物化表、字段、转换节点，对共享逻辑去重，并融合跨 SQL 的表级血缘。
...
├── visualize/   # Cytoscape.js HTML 渲染（ELK 布局、主题）
